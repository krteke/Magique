...
impl Png {
    ...
    pub fn search_chunk(&self, chunk_type: &str) -> Result<String, anyhow::Error> {
        let chunk_type = ChunkType::from_str(chunk_type)?;

        let chunk = self.chunks.iter().rfind(|x| chunk_type == *x.chunk_type());

        if let Some(a) = chunk {
            Ok(String::from_utf8(a.data().to_vec()).unwrap())
        } else {
            Err(anyhow!("no chunk found"))
        }
    }

    pub fn append_chunk(&mut self, chunk: Chunk) {
        let end = self.chunks.pop().unwrap();
        self.chunks.push(chunk);
        self.chunks.push(end);
    }
    ...
    pub fn as_bytes(&self) -> Vec<u8> {
        ...
        self.chunks().iter().for_each(|iter| {
            bytes.append(&mut iter.as_bytes());
        });
        bytes
    }
}
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self> {
        ...
        while !remaining_bytes.is_empty() {
            ...
            let chunk_size = chunk.length() as usize + 12;
